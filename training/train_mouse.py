...
dim = 50
f_net = FNet_interaction(in_out_dim=dim, hidden_dim=400, n_hiddens=2, activation='leakyrelu', thre=300)
...
df = pd.read_csv(DATA_DIR + '/Weinreb.csv')
...
if n_local_epochs > 0:
    ...
    for epoch in tqdm(range(1), desc='Training Epoch'):
        l_loss, b_loss, g_loss = train_all(
            f_net, df, groups, optimizer, 1000,
            criterion=criterion, use_cuda=use_cuda,
            local_loss=True, global_loss=False, apply_losses_in_time=apply_losses_in_time,
            ...
            exp_dir=exp_dir, scheduler=scheduler, thre=1000, lambda_ot=10, lambda_mass=10, lambda_energy=0.01, log_mass=True, sf2m_optimizer=sf2m_optimizer
        )
