...
from CytoBridge.losses import MMD_loss, OT_loss1, OT_loss2_spatial, OT_loss3_spatial, Density_loss, Local_density_loss
...
dim = 52
f_net = FNet_interaction(in_out_dim=dim, hidden_dim=400, n_hiddens=2, activation='leakyrelu', thre=0.3, spatial=True)
...
df = pd.read_csv(DATA_DIR + '/mosta_three_time.csv')
...
opts['criterion'] = 'ot1'
criterion = OT_loss2_spatial(which='emd', alpha_spatial=10, alpha_express=0.02)
...
if n_local_epochs > 0:
    ...
    for epoch in tqdm(range(1), desc='Pretraining Epoch'):
        l_loss, b_loss, g_loss = train_un1_reduce_interaction(
            ...
            top_k=top_k, sample_size=sample_size, relative_mass=relative_mass, initial_size=initial_size,
            sample_with_replacement=sample_with_replacement, logger=logger, device=device, best_model_path=exp_dir+'/best_model',
            pca_transform=None, reverse=False, thre=1000, use_spatial=True
        )
...
device = 'cuda'
optimizer = torch.optim.Adam(list(f_net.parameters()), 1e-4)
sf2m_optimizer = torch.optim.Adam(list(sf2m_score_model.parameters()), 1e-4)
...
criterion = OT_loss3_spatial(which='emd', alpha_spatial=10, alpha_express=0.02)
...
if n_local_epochs > 0:
    ...
    for epoch in tqdm(range(1), desc='Training Epoch'):
        l_loss, b_loss, g_loss = train_all(
            ...
            exp_dir=exp_dir, scheduler=scheduler, thre=1000, lambda_ot=10, lambda_mass=10, lambda_energy=0.01, log_mass=True, sf2m_optimizer=sf2m_optimizer, use_spatial=True
        )
