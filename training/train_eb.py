...
dim = 50
f_net = FNet_interaction(in_out_dim=dim, hidden_dim=400, n_hiddens=2, activation='leakyrelu', thre=100)
...
df = pd.read_csv(DATA_DIR + '/eb.csv')
df = df.iloc[:, :51]
...
device = 'cuda'
optimizer = torch.optim.Adam(list(f_net.parameters())+list(sf2m_score_model.parameters()), 1e-4)
...
if n_local_epochs > 0:
    ...
    for epoch in tqdm(range(1), desc='Training Epoch'):
        l_loss, b_loss, g_loss = train_all(
            ...
            exp_dir=exp_dir, scheduler=scheduler, thre=1000, lambda_ot=10, lambda_mass=10, lambda_energy=0.01, log_mass=True
        )
